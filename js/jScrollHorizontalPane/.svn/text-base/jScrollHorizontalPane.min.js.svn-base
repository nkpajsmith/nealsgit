 ...
 * *******
 * 
 * This plugin is derived in part from jScrollPaneHorizontal created by Kevin Luck(http://www.kelvinluck.com)
 * 
 * Copyright (c) 2006 Kelvin Luck (kelvin AT kelvinluck DOT com || http://www.kelvinluck.com)
 * Dual licensed under the MIT (http://www.opensource.org/licenses/mit-license.php) 
 * and GPL (http://www.opensource.org/licenses/gpl-license.php) licenses.
 * 
 * See http://kelvinluck.com/assets/jquery/jScrollPaneHorizontal/
 * $Id: jScrollPaneHorizontal.js 3125 2007-09-06 20:39:42Z kelvin.luck $
 */

/**
 * Replace the default horizontal scroll bars on matched 
 * elements with a CSS styled veresion.  Very similar to the jScrollPaneHorizontal 
 ...
 * 
 *
 * @example jQuery(".scroll-pane").jScrollPaneHorizontal();
 *
...
jQuery('>.jScrollPaneHorizontalTrack, >.jScrollArrowLeftHorizontal, >.jScrollArrowRightHorizontal',$c).remove();$this.css({'left':0});_jscr_originalPos[currentId]=-1;}else{var currentScrollPosition=0;this.originalPadding=$this.css('paddingTop')+' '+$this.css('paddingRight')+' '+$this.css('paddingBottom')+' '+$this.css('paddingLeft');this.originalSidePaddingTotal=(parseInt($this.css('paddingLeft'))||0)+(parseInt($this.css('paddingRight'))||0);var paneWidth=$this.outerWidth();var rightPos=$this.offset().left+_jscr_originalPos[currentId]+paneWidth;if((rightPos)>$(window).width()){paneWidth=$(window).width()*percentageWidth;}
if(paneWidth<settings.minimumWidth){paneWidth=settings.minimumWidth;}
var paneHeight=$this.innerHeight();var trackWidth=paneWidth;$this.wrap(jQuery('<div></div>').attr({'className':'jScrollPaneHorizontalContainer'}).css({'height':paneHeight+'px','width':paneWidth+'px'}));jQuery(document).bind('emchange',function(e,cur,prev)
...
var percentInView=paneWidth/contentWidth;var trackIntervals=new Array();if(percentInView<0.99){var $container=$this.parent();$container.append(jQuery('<div></div>').attr({'className':'jScrollPaneHorizontalTrack'}).css({'height':settings.scrollbarHeight+'px'}).append(jQuery('<div></div>').attr({'className':'jScrollPaneHorizontalDrag'}).css({'height':settings.scrollbarHeight+'px'}).append(jQuery('<div></div>').attr({'className':'jScrollPaneHorizontalDragLeft'}).css({'height':settings.scrollbarHeight+'px'}),jQuery('<div></div>').attr({'className':'jScrollPaneHorizontalDragRight'}).css({'height':settings.scrollbarHeight+'px'}))));var $track=jQuery('>.jScrollPaneHorizontalTrack',$container);for(inter in _jscr_intervals){if(settings.showArrows==true){scrollOffset=settings.arrowSize;}else{scrollOffset=0;}
...
if(inter!=0){interObj=jQuery('<div>|</div>').attr({'className':'jScrollIntervalTrackHorizontal'}).css({'left':intervalTrackPos+'px'})
$track.append(interObj);}}
var $drag=jQuery('>.jScrollPaneHorizontalTrack .jScrollPaneHorizontalDrag',$container);if(settings.showArrows){var currentArrowButton;var currentArrowDirection;var currentArrowInterval;var currentArrowInc;var whileArrowButtonDown=function()
{if(currentArrowInc>4||currentArrowInc%4==0){positionDrag(dragPosition+currentArrowDirection*mouseWheelMultiplier);}
currentArrowInc++;};var onArrowMouseUp=function(event)
{jQuery('body').unbind('mouseup',onArrowMouseUp);currentArrowButton.removeClass('jScrollActiveArrowButtonHorizontal');clearInterval(currentArrowInterval);arrowUp=true;moveIntervals();};var onArrowMouseDown=function(){jQuery('body').bind('mouseup',onArrowMouseUp);currentArrowButton.addClass('jScrollActiveArrowButtonHorizontal');currentArrowInc=0;whileArrowButtonDown();currentArrowInterval=setInterval(whileArrowButtonDown,100);};$container.append(jQuery('<a></a>').attr({'href':'javascript:;','className':'jScrollArrowLeftHorizontal'}).css({'width':settings.arrowSize+'px'}).html('Scroll Left').bind('mousedown',function()
{currentArrowButton=jQuery(this);currentArrowDirection=-1;onArrowMouseDown();this.blur();return false;}),jQuery('<a></a>').attr({'href':'javascript:;','className':'jScrollArrowRightHorizontal'}).css({'width':settings.arrowSize+'px'}).html('Scroll Right').bind('mousedown',function()
{currentArrowButton=jQuery(this);currentArrowDirection=1;onArrowMouseDown();this.blur();return false;}));if(settings.arrowSize){trackWidth=paneWidth-settings.arrowSize-settings.arrowSize;$track.css({'width':trackWidth+'px',left:settings.arrowSize+'px'})}else{var leftArrowWidth=jQuery('>.jScrollArrowLeftHorizontal',$container).width();settings.arrowSize=leftArrowWidth;trackWidth=paneWidth-leftArrowWidth-jQuery('>.jScrollArrowRightHorizontal',$container).width();$track.css({'width':trackWidth+'px',left:leftArrowWidth+'px'})}}
...
if((parseInt($drag.width())+parseInt(trackIntervals[_jscr_trackInt[currentId]]))>parseInt($('.jScrollPaneHorizontalTrack').width())){movePos=parseInt($('.jScrollPaneHorizontalTrack').width())-$drag.width();}}else{_jscr_trackInt[currentId]=parseInt(_jscr_trackInt[currentId])-1;if(_jscr_trackInt[currentId]<0){_jscr_trackInt[currentId]=0;}}}
